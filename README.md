...
* Build images that contain all the dependencies of a project to run in a container-supporting cloud environment
* Build images that contain a project in its completely run state.
...
The package must be at the root of a git repository, but you'll want to run dockertest from within a subdirectory (e.g., `docker/`) because we copy files and clone some repos around.  Add to your `.Rbuildignore` a line that says

```
^docker$
```

to keep `R CMD check` happy.  Then run:
...
will build an image that contains project dependencies, and clone your project into `/root/<project_name>` (which will be the working directory when the container starts).  You will have to specify depenencies however -- see the [Configuration](#configuration) section.
...
* `keep_git:` (`true` / `false`) -- Keep the .git directory in the
