...
prepare_run <- function(suffix="-run") {
  ...
  if (info$path_project == getwd()) {
    stop("Can't run this in the project root")
  }
  if (file.exists(dest_src)) {
    unlink(dest_src, recursive=TRUE)
  }
  system2("git", c("clone", info$path_project, dest_src))
  system2("git", c(sprintf("--git-dir=%s/.git", dest_src),
                   "remote", "rm", "origin"))
  if (system2("git", c("check-ignore", dest_src), stderr=FALSE) != 0L) {
    write(sprintf("%s/", dest_src), ".gitignore", append=TRUE)
  }
}
